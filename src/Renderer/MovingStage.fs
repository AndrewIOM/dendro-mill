...
module FakeStage =
    ...
    let moveAction : MoveMotor =
        fun (step:int<step>) ->
            promise {
                do! Fable.PowerPack.Promise.sleep (1 * step |> removeUnit)
                match rand.Next(1,250) with
                | 5 -> return HitLowerBound
                | _ -> return Finished }

    let activateAxis axis =
        match axis with
        | X -> Axis.linear 1200<step> None moveAction |> Axis.Linear
        | Y -> Axis.linear 1200<step> None moveAction |> Axis.Linear
        | Vertical -> Axis.linear 1200<step> None moveAction |> Axis.Linear
        | Rotation -> Axis.tilting 100<step> None moveAction |> Axis.Tilt
        | StageAngle -> Axis.tilting 200<step> None moveAction |> Axis.Tilt

    let connect () =
        promise {
            let status = Connected { X = activateAxis X; Y = activateAxis Y; Vertical = activateAxis Vertical; Tilt = activateAxis StageAngle }
            return status }


//////////////////////////
/// Arduino-Powered Stage
//////////////////////////
...
module Arduino =
    ...
    module Stepper =
        ...
        let toMoveAction' (stepper:Stepper) =
            fun (steps:int<step>) cb -> 
                stepper.step(
                    createObj [ 
        ...
        let toMoveAction (stepper:Stepper) (steps:int<step>) = 
            fun onSuccess -> 
                promise {
                    (stepper |> toMoveAction') steps (System.Func<unit,unit>(onSuccess))
                }
